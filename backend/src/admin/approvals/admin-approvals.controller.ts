...
// Approval queue is admin-only (decisions). Kurye Şefi/Müdür cannot self-approve.
...
export class AdminApprovalsController {
  ...
  @Patch('shift-changes/:id')
  decideShiftChange(@Param('id') id: string, @Body() dto: ApprovalDecisionDto, @CurrentUser() user: AuthUser) {
    return this.service.decideShiftChange(id, dto, user.userId);
  }
}
