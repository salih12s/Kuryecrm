...
export default function DailyReportPage() {
  ...
  useEffect(() => { load(); }, []);
  const csv = () => data && exportCsv(`gun-sonu-${date}.csv`, ['Tarih','Restoran','Kurye','Saat','Çalışma Saati','Hizmet Bedeli','Kurye Hak Edişi'], data.shifts.map(s => [s.date,s.restaurantName,s.courierName,timeRange(s.approvedStartTime,s.approvedEndTime),s.workHours,s.restaurantServiceAmount,s.courierEarning]));
  return <AdminLayout><div className="mb-6 flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-2xl font-bold text-primary">Gün Sonu Raporu</h1><p className="mt-1 text-sm text-muted">Tek bir günün operasyon ve nakit hareketleri.</p></div><ReportActions onExport={data ? csv : undefined} /></div>
    ...
    {data && !loading && <div className="space-y-6"><h2 className="font-semibold text-text">{formatDateTR(data.date)}</h2><ReportSummaryCards summary={data.summary} />
      ...
      <SimpleSection title="Onaylı Vardiyalar"><table className="w-full text-sm"><thead><tr>{['Tarih','Restoran','Kurye','Onaylı Saat','Süre','Hizmet','Hak Ediş'].map(h=><th key={h} className="px-4 py-3 text-left">{h}</th>)}</tr></thead><tbody>{data.shifts.length ? data.shifts.map(s=><tr key={s.id} className="border-t"><td className="px-4 py-3">{formatDateTR(s.date)}</td><td className="px-4">{s.restaurantName}</td><td className="px-4">{s.courierName}</td><td className="px-4">{timeRange(s.approvedStartTime,s.approvedEndTime)}</td><td className="px-4">{s.workHours} sa</td><td className="px-4">{formatTL(s.restaurantServiceAmount)}</td><td className="px-4">{formatTL(s.courierEarning)}</td></tr>):<Empty col={7}/>}</tbody></table></SimpleSection>
